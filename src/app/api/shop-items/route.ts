...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, subtitle, price, badge, images, sizes, description, payment, shipping } = body;
    ...
    const item = await prisma.shopItem.create({
      data: { name, subtitle, price: Number(price), badge, images, sizes, description, payment, shipping },
    });
